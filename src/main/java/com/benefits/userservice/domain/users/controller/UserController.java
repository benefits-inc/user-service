...
@Tag(name="Users - Supervisor 권한 필요", description = "Supervisor(관리자용) API 입니다. 요청 시 관리자의 권한(토큰)이 필요합니다.")
public class UserController {
    ...
    @Operation(summary = "관리자의 권한으로 전체 사용자 조회", description = "회원 상태가 탈퇴 처리, 탈퇴 진행, 탈퇴 처리 된 회원까지 전부 조회")
    @GetMapping("/users")
    ...
    @Operation(summary = "관리자의 권한으로 특정 사용자 조회", description = "관리적 측면의 회원의 상세 정보가 조회 되며 (주소, 전화번호 ) / 회원 상태가 탈퇴 처리, 탈퇴 진행, 탈퇴 처리 된 회원까지 전부 조회 가능")
    @GetMapping("/users/{id}")
    ...
    @Operation(summary = "관리자의 권한으로 특정 사용자 삭제 - soft delete")
